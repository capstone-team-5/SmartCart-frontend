//This page will display breakfast foods
import { Link } from "react-router-dom";

const BreakfastFoodComponent = () => {
    return (
        <div className="bg-gray-100 min-h-screen">
            <div className="container mx-auto py-8">
            <h1 className="text-4xl font-semibold text-center mb-8">
                    Add the Ingredients List to Your Cart
                </h1>
        <div className="grid grid-cols-3 gap-4 mt-8">
            {/* Recipe Card 1 */}
            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/breakfast-food-cheese-omelette" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="https://stordfkenticomedia.blob.core.windows.net/df-us/rms/media/recipemediafiles/recipes/retail/desktopimages/15492.jpg?ext=.jpg"
                        alt="Cheese Omelette"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Cheese Omelette</strong>
                </Link>
            </div>

            {/* Recipe Card 2 */}
            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/breakfast-food-chocolate-chip-pancakes" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="https://tornadoughalli.com/wp-content/uploads/2019/07/CHOCOLATE-CHIP-PANCAKES-12.jpg"
                        alt="Chocolate Chip Pancakes"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Chocolate Chip Pancakes</strong>
                </Link>
            </div>

            {/* Recipe Card 3 */}
            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/breakfast-food-overnight-oats" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ-p2YyUxUxX6IYTDLn-HUYPZklphkU6W1oQg&usqp=CAU"
                        alt="Overnight Oats"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Overnight Oats</strong>
                </Link>
            </div>

            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/breakfast-food-blueberry-waffles" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="https://bakingmischief.com/wp-content/uploads/2021/02/blueberry-waffles-image-square-4.jpg"
                        alt="Blueberry Waffles"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Blueberry Waffles</strong>
                </Link>
            </div>

            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/breakfast-food-keto-chocolate-croissant" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="https://www.wholesomeyum.com/wp-content/uploads/2021/04/wholesomeyum-keto-croissant-recipe-chocolate-filling-22-500x500.jpg"
                        alt="Keto Chocolate Croissant"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Keto Chocolate Croissant</strong>
                </Link>
            </div>

            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/breakfast-food-breakfast-burrito" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="https://www.onceuponachef.com/images/2018/03/Breakfast-Burritos.jpg"
                        alt="Breakfast Burrito"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Breakfast Burrito</strong>
                </Link>
            </div>




            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/breakfast-food-spiced-cream-of-wheat-porridge" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="data:image/jpeg;base64,"
                        alt="Spiced Cream of Wheat Porridge"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Spiced Cream of Wheat Porridge</strong>
                </Link>
            </div>

            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/old-fashion-oats" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="https://www.eatingwell.com/thmb/-UULlbERQCfJRQTnb5bwjoo9-UQ=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/old-fashioned-oatmeal-hero-05-060861b81cb641cea272e068aba093fd.jpg"
                        alt="Old Fashion Oats"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Old Fashion Oats</strong>
                </Link>
            </div>

            <div className="bg-white rounded-lg shadow-md p-4">
                <Link to="/recipes/breakfast-food-yogurt-parfait" className="text-blue-500 hover:text-blue-700">
                    <img
                        src="https://simplyhomecooked.com/wp-content/uploads/2021/08/yogurt-parfait-4.jpg"
                        alt="Yogurt Parfait"
                        className="w-full h-40 object-cover rounded-t-lg"
                    />
                    <strong className="text-lg">Yogurt Parfait</strong>
                </Link>
            </div>

                </div>
                </div>
            </div>
    );
};

export default BreakfastFoodComponent;
