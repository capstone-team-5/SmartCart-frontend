...
const CheeseOmeletteRecipeComponent = ({
  ...
  useEffect(() => {
    // console.log("cheese specificIds", specificIds);
    if (specificIds && specificIds.length > 0) {
      // Use a filter function to select items with specific IDs
      const filtered = allItems.filter((item) =>
        specificIds.includes(item.product_id)
  ...
  const handleAddToCartClick = () => {
    try {
      ...
      if (addToCart && filteredItems.length > 0) {
        // console.log("Filtered Items chesse:", filteredItems);

        filteredItems.forEach((product) => {
          // Add the product to the cart with the desired properties

          console.log("Adding to cart chese:", product);
          console.log("name of food hese:", product.product_name);
          console.log("image of food chese:", product.product_image);
          console.log("id chesse :", product.product_id);
          const food = {
            name: product.product_name,
            image: product.product_image,
            id: product.product_id,
          ...
          // food.id
          // food.product_id
          newItemAddedToFood.push(food); // [ {1,milk}, {2, egg}, {3, milk} ]
          // const result = newItemAddedToFood(food)
        });

        addToCart(newItemAddedToFood);
        console.log(
          "Result of addToCart cheese:",
          newItemAddedToFood,
          addToCart(newItemAddedToFood)
        );
